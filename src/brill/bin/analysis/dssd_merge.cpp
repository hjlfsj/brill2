...
int main(int argc, char **argv) {
	...
	if (glimmer::ParseProgramOptions(argc, argv, options, true, false) != 0 || options.help) {
		glimmer::PrintGeneralUsage(argv[0], true, false);
	...
	return glimmer::RunDssdMerge(config, options.detector, options.trigger, options.run) == 0 ? 0 : 1;
}
